...
switch($requestMethod) {
	case 'GET':
		...
		$controlInfo = $control->getTrakedTime();
		...
		foreach (reset($_queryActivityLog) as $key => $data) {
			foreach ($data as $f_key => $f_val) {
				...
				// $changedAt = date_create(strtotime($datadecoded['changed_at']), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");


				$startDate = date_create($_startDate, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");
				// $changedAt = date_create($_changedAt, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");
				$actualDate = date_create(date(), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");
				...
				foreach ($aditionalValue as $g_key => $g_value){
					...
					$updatedAt = date_create(strtotime($g_value['updated_at']), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");
					$startedAt = date_create(strtotime($g_value['started_at']), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");
					$endedAt = date_create(strtotime($g_value['ended_at']), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");
					$createdAt = date_create(strtotime($g_value['created_at']), new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y");


					// $_startedAt = date('d-m-Y H:i:s', $g_value['started_at']);
					// $startedAt = date_create($_startedAt, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");

					// $_endedAt = date('d-m-Y H:i:s', $g_value['ended_at']);
					// $endedAt = date_create($_endedAt, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");

					// $_createdAt = date('d-m-Y H:i:s', $g_value['created_at']);
					// $createdAt = date_create($_createdAt, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");

					// $_updatedAt = date('d-m-Y H:i:s', $g_value['updated_at']);
					// $updatedAt = date_create($_updatedAt, new DateTimeZone($TimeZoneNameFrom))->setTimezone(new DateTimeZone($TimeZoneNameTo))->format("d-m-Y H:i:s");
					 //echo "|=>";
					// print_r($updatedAt);

					//  if($yesterday > $startedAt){
					// 	$tracked = false;
					// } else {
						$tracked = true;
