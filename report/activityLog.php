			...
			foreach ($data as $f_key => $f_val) {
				...
				foreach ($items as $key => $value) {
					...
					foreach ($aditionalValue as $key => $valueo){
						...
						$endPeriod = isset($_GET["startToday"]) ? date('d-m-Y') : $endPeriod = $configApp['dayEndPeriod'] . '-' . $currentMonth;
						$startedPeriod = $configApp['dayStartPeriod'] . '-' . $startedCurrentMonth; 
						...
						if($boolPeriod){
							...
							$dateEndPeriod= $configApp['dayEndPeriod'] . '-' . $monthEndPeriod; 
							$dateStartedPeriod  = $configApp['dayStartPeriod'] . '-' . $monthStartedPeriod; 
							...
							$horasExtraDecimal = Functions::decimalHours($horasExtras);
							if($horasExtraDecimal > 0){
								$activityList[] = array(
									"boardId" => $boardId,
									"itemId" => $itemId,
									"entryId" => $entryId,
